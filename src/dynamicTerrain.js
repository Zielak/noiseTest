...
class DynamicTerrain {
  ...
  _updateTerrain(updateSize) {
    ...
    for (var j = 0 | 0; j <= terrainSub; j++) {
      ...
      for (var i = 0 | 0; i <= terrainSub; i++) {
        ...
        // map current index
        index =
          mod(deltaSubZ + stepJ, mapSubZ) * mapSubX +
          mod(deltaSubX + stepI, mapSubX)
        terIndex =
   ...
   * If the optional object {normal: Vector3} is passed, then its property "normal" is updated with the normal vector value at the coordinates (x, z).
   */
  static GetHeightFromMap(x, z, mapData, mapSubX, mapSubZ, options, inverted) {
    var mapSizeX = Math.abs(mapData[(mapSubX - 1) * 3] - mapData[0])
    var mapSizeZ = Math.abs(
   ...
   */
  static ComputeNormalsFromMapToRef(
    mapData,
    mapSubX,
    mapSubZ,
    normals,
   ...
   */
  static CreateMapFromHeightMap(heightmapURL, options, scene) {
   ...
   */
  static CreateMapFromHeightMapToRef(heightmapURL, options, data, scene) {
   ...
   */
  static CreateUVMapToRef(subX, subZ, mapUVs) {
   ...
   */
  get refreshEveryFrame() {
    return this._refreshEveryFrame
  }
  set refreshEveryFrame(val) {
   ...
   */
  get cameraLODCorrection() {
    return this._cameraLODCorrection
  }
  set cameraLODCorrection(val) {
   ...
   * Default false
   */
  get useCustomVertexFunction() {
    return this._useCustomVertexFunction
  }
  set useCustomVertexFunction(val) {
   ...
   * Default false.
   */
  get precomputeNormalsFromMap() {
    return this._precomputeNormalsFromMap
  }
  set precomputeNormalsFromMap(val) {
   ...
   */
  beforeUpdate(refreshEveryFrame) {
    return
  }
  /**
  ...
  // tmp vectors
  static _v1 = Vector3.Zero()
  static _v2 = Vector3.Zero()
  static _v3 = Vector3.Zero()
  static _v4 = Vector3.Zero()
  static _vAvB = Vector3.Zero()
  static _vAvC = Vector3.Zero()
  static _norm = Vector3.Zero()
  static _bbMin = Vector3.Zero()
  static _bbMax = Vector3.Zero()
}
